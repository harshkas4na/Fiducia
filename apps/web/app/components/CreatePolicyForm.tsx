...
const CreatePolicyForm: React.FC = () => {
  ...
  useEffect(() => {
    const ConnectContract=async()=>{
      ...
      const contract = new web3.eth.Contract(CryptoInsurance_ABI as any, CryptoInsurance_ADDRESS);
  ...
  const handleCreatePolicy = async () => {
    ...
    switch (selectedType) {
      case "0": // LOAN
        premiumRate = await InsuranceContract.methods.LOAN_PREMIUM_RATE().call();
        break;
      case "1": // THRESHOLD
        premiumRate = await InsuranceContract.methods.THRESHOLD_PREMIUM_RATE().call();
        break;
      case "2": // SUDDEN_DROP
        premiumRate = await InsuranceContract.methods.SUDDEN_DROP_PREMIUM_RATE().call();
        break;
      default:
        throw new Error("Invalid insurance type");
    }

    const premiumBN = coverageAmountBN.mul(new BN(premiumRate)).div(new BN(10000));
    const premiumWei = premiumBN.toString();

      const transaction = await InsuranceContract.methods.createPolicy(
        assetAddress,
        selectedType,
        coverageAmountWei,
        triggerPrice
  ...
  const getAssetAddress = (asset: string) => {
    ...
    return assetAddresses[asset] || "0x0000000000000000000000000000000000000000";
  };
